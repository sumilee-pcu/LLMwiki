...
| `apps/local-ai-workbench/` | Ollama 로컬 모델과 LLMwiki 문서를 연결하는 웹 기반 작업실 |
...
## 6. Local AI Workbench 사용법

Local AI Workbench는 LLMwiki 문서, MCP 논문 초안, GitHub 도구 조사 문서를 로컬 Ollama 모델에 바로 연결하기 위한 웹 기반 작업실입니다. 일반 채팅창이 아니라 현재 저장소의 지식 구조를 문맥으로 넣을 수 있게 만든 작은 하네스입니다.
...
1. 왼쪽에서 `채팅`, `LLMwiki`, `MCP 논문`, `도구 조사` 모드를 선택합니다.
2. 상단의 모델 선택에서 `gemma3:27b`, `gemma3n:e4b`, `llama3.3:70b-instruct-q4_K_M`, `phi4:14b` 중 하나를 고릅니다.
3. LLMwiki, MCP 논문, 도구 조사 모드에서는 `현재 모드 문맥 포함`을 켜면 로컬 문서 내용이 함께 전달됩니다.
4. 빠른 작업 버튼으로 자주 쓰는 점검 프롬프트를 불러오거나 직접 요청을 입력합니다.

권장 모델 배치:

| 작업 | 권장 모델 | 이유 |
| --- | --- | --- |
| 위키 정리, 긴 한국어 설명, 논문 리뷰 | `gemma3:27b` | 현재 로컬 주력 모델이며 긴 컨텍스트와 비전 입력을 지원합니다. |
| 짧은 요약, 빠른 초안, 상태 확인 | `gemma3n:e4b` | 응답이 빠르고 가벼운 작업에 적합합니다. |
| 무거운 추론 백업 | `llama3.3:70b-instruct-q4_K_M` | 속도보다 품질이 중요할 때 비교 모델로 사용합니다. |
| 코드/논리 비교 테스트 | `phi4:14b` | 코드성 질의와 논리 점검용 후보로 유지합니다. |

이 웹앱의 효과는 세 가지입니다.

- LLMwiki 자료를 복사해서 붙여 넣는 시간을 줄입니다.
- 로컬 모델이 실제 프로젝트 문맥을 읽고 답하게 만들 수 있습니다.
- Open WebUI나 AnythingLLM을 도입하기 전, 내 작업 흐름에 맞는 화면과 프롬프트를 먼저 검증할 수 있습니다.
...
- 로컬 점검 도구 작동 확인
- 로컬 커밋 완료
- Local AI Workbench 웹앱 추가 및 Ollama 연동 확인
- GitHub push는 권한 문제로 보류

최근 로컬 커밋:

```text
96f2d46 Add useful GitHub repository links
```
...
- Obsidian 설정 파일 추가
- 주제별 대시보드 문서 추가
- Local AI Workbench 대화 저장 기능 추가
- Local AI Workbench 문서 검색/RAG 기능 추가
