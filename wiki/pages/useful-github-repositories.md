...
created: 2026-05-18
updated: 2026-05-28
tags: [github, tools, mcp, ai-tools, vibe-coding, skillopt, legal-tech, 12-factor-agents]
sources:
...
| korean-law-mcp | https://github.com/chrisryugj/korean-law-mcp | 법령·규칙·판례 검색 MCP | 법제처 API 기반, 조문·판례 검색, 인용 검증 강조. 원격 MCP 주소 `https://korean-law-mcp.fly.dev/mcp`는 API 키 없이 연결 가능. 로컬 설치는 `npm install -g korean-law-mcp` 후 open.law.go.kr에서 무료 API 키 발급 필요. |
| kordoc | https://github.com/chrisryugj/kordoc | HWP/PDF 파싱 부가 도구 | 법령·행정 문서 원문 처리 시 HWP/PDF 파싱 보조 후보로 함께 검토. |

### korean-law-mcp 연결 메모

| 방식 | 주소·명령 | API 키 | 메모 |
| --- | --- | --- | --- |
| 원격 MCP | `https://korean-law-mcp.fly.dev/mcp` | 불필요 | 빠른 연결 테스트와 Claude/Codex류 MCP 클라이언트 연동 후보 |
| 로컬 설치 | `npm install -g korean-law-mcp` | 필요 | API 키는 [국가법령정보 공동활용](https://open.law.go.kr)에서 무료 발급 |
...
| 이름 | 링크 | 용도 | 메모 |
| --- | --- | --- | --- |
| 12-Factor Agents | https://github.com/humanlayer/12-factor-agents | 프로덕션급 LLM 에이전트 설계 원칙 | HumanLayer 공개 가이드. context window 소유, human-in-the-loop tool call, small focused agents, stateless reducer 등 하네스 설계 원칙과 직접 연결된다. 상세 정리는 [[12-Factor Agents]] 참고. |
...
1. `korean-law-mcp`: 한국 법령·규칙·판례 검색을 LLM 작업 흐름에 붙일 수 있다. 원격 MCP는 API 키 없이 빠르게 테스트하고, 로컬 설치는 open.law.go.kr API 키를 발급해 사용한다.
...
7. `12-factor-agents`: 프로토타입 이후 70-80% 품질 벽을 넘기 위한 에이전트 소프트웨어 설계 원칙으로 가치가 있다.
